...
usage() {
    ...
    echo ""
    echo "This script obtains/renews Let's Encrypt certificates using certbot with the dns-cloudflare plugin,"
...
if [ ! -d "$VENV_PATH" ]; then
    ...
    echo "Upgrading pip and installing packages (certbot, certbot-dns-cloudflare)..."
    "${VENV_PATH}/bin/pip" install --upgrade pip
    "${VENV_PATH}/bin/pip" install certbot==2.11.1 certbot-dns-cloudflare
    if [ $? -ne 0 ]; then
...
# Check if cloudflare.ini exists and has correct permissions
if [ ! -f "$CREDENTIALS_FILE" ]; then
    echo "Error: cloudflare.ini file not found. Please create it with your Cloudflare DNS API Token:"
